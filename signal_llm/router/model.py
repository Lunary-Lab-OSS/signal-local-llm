...
# Now safe to import transformers/sentence-transformers
...
class ResidualSwiGLUBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # LayerNorm AFTER SwiGLU (Post-LN style)
        out = self.layer_norm(gated)

        # Residual connection
        residual = self.residual_proj(x) if self.residual_proj is not None else x
        return out + residual  # Residual connection: F(x) + x
...
class SingleTowerStudent(nn.Module):
    def __init__(
        ...
        if hasattr(self.backbone, "_modules") and "0" in self.backbone._modules:
            ...
            if hasattr(underlying_model, "tokenizer"):
                ...
                # Tokenizer will use token from huggingface_hub if we've logged in
        ...
        # --- QUANTIZATION MAGIC HAPPENS HERE (AFTER FULL INITIALIZATION) ---
        # Apply INT4 weight-only quantization to the underlying transformer model
        # NOTE: We apply quantization AFTER head creation to avoid PyTorch internal operations
        # that trigger shallow_copy on AffineQuantizedTensor during model structure setup
        # Note: INT4 works on both CPU and CUDA, but is most efficient on CUDA GPUs
        if use_int4:
            if TORCHAO_AVAILABLE:
                if self.device.type == "cpu":
                    logger.warning(
                        "⚠️  INT4 quantization on CPU: Works but may be slower than FP32/FP16 due to lack of optimized kernels"
                    )
                else:
                    logger.info("Applying INT4 Weight-Only Quantization via TorchAO...")
                try:
                    # SentenceTransformer wraps the model in _modules['0']
                    # Access the underlying AutoModel
                    if hasattr(self.backbone, "_modules") and "0" in self.backbone._modules:
                        underlying_model = self.backbone._modules["0"]
                        # Check if it has an auto_model attribute (SentenceTransformer structure)
                        if hasattr(underlying_model, "auto_model"):
                            transformer_model = underlying_model.auto_model
                        else:
                            # Direct model access
                            transformer_model = underlying_model

                        # Apply INT4 quantization to the transformer model
                        # group_size=32 is optimal for RTX 4090 (Ada Lovelace architecture)
                        # inner_k_tiles=8 is for CUDA optimization (may not help on CPU)
                        # Note: There's a known issue with AffineQuantizedTensor shallow_copy in some PyTorch/torchao versions
                        # We'll try quantization but fall back gracefully if it fails
                        try:
                            if self.device.type == "cuda":
                                # Try with inner_k_tiles first (faster on CUDA)
                                try:
                                    apply_quantization = int4_weight_only(
                                        group_size=32, inner_k_tiles=8
                                    )
                                    quantize_(transformer_model, apply_quantization)
                                    logger.info(
                                        "   ✅ INT4 quantization applied successfully on CUDA (group_size=32, inner_k_tiles=8)"
                                    )
                                except Exception as inner_k_error:
                                    error_str = str(inner_k_error)
                                    if (
                                        "shallow_copy" in error_str
                                        or "AffineQuantizedTensor" in error_str
                                        or "unimplemented operator" in error_str
                                    ):
                                        # Known compatibility issue - retry without inner_k_tiles
                                        logger.warning(
                                            f"INT4 quantization with inner_k_tiles failed (known compatibility issue), retrying without: {inner_k_error}"
                                        )
                                        apply_quantization = int4_weight_only(group_size=32)
                                        quantize_(transformer_model, apply_quantization)
                                        logger.info(
                                            "   ✅ INT4 quantization applied successfully on CUDA (group_size=32, no inner_k_tiles)"
                                        )
                                    else:
                                        raise
                            else:
                                # CPU: don't use inner_k_tiles (may not be supported/optimized)
                                apply_quantization = int4_weight_only(group_size=32)
                                quantize_(transformer_model, apply_quantization)
                                logger.info(
                                    "   ✅ INT4 quantization applied successfully on CPU (group_size=32)"
                                )
                            ...
                            if (
                                ...
                                # Known torchao/PyTorch compatibility issue - continue with unquantized model
                                logger.warning(
                                    f"   ⚠️  INT4 quantization failed due to compatibility issue: {quant_error}"
                                )
                                logger.warning(
                                    "   ⚠️  Continuing with unquantized model (will be slower but functional)"
                                )
                                # Continue with unquantized model - it will still work
                            else:
                                logger.warning(
                                    f"   ⚠️  INT4 quantization failed: {quant_error}. Continuing with unquantized model."
                                )

                        device_str = "CUDA" if self.device.type == "cuda" else "CPU"
                        logger.info(
                            f"   ✅ INT4 quantization applied successfully on {device_str} (group_size=32)"
                        )
                    else:
                        logger.warning("   ⚠️  Could not access underlying model for quantization")
                except Exception as e:
                    logger.warning(
                        f"   ⚠️  INT4 quantization failed: {e}. Continuing with unquantized model."
                    )
            else:
                logger.warning("   ⚠️  TorchAO not available, skipping INT4 quantization")
    ...
    def forward(self, texts):
        ...
        # 2. Backbone Forward (INT4 on GPU)
        # Use SentenceTransformer's encode which handles tokenization and pooling
        # CRITICAL: normalize_embeddings must match pre-computed embeddings in database
        with torch.no_grad():
            ...
            # SentenceTransformer returns [batch, emb_dim] already pooled

        # 3. Fusion
        combined = torch.cat([pooled_emb, ling_feats], dim=1)
